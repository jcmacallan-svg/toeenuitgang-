...
(() => {
  ...
  // NEW Training ID Card (inside idCardWrap)
  const trainerIdCard = $("#trainerIdCard");
  const idPhoto2 = $("#idPhoto2");
  const idName2 = $("#idName2");
  const idDob2 = $("#idDob2");
  const idNat2 = $("#idNat2");
  const idNo2 = $("#idNo2");
  const idReason2 = $("#idReason2");
  const idExp2 = $("#idExp2");
  ...
  // Decide whether ID photo differs from face photo (default same).
  function buildPhotoPair(baseIdx, baseSrc){
    const facePhotoSrc = baseSrc || headshotPath(baseIdx);
    let idPhotoSrc = facePhotoSrc;

    // Use mood liarBias as chance for mismatch (fake/incorrect ID photo)
    const p = Math.max(0, Math.min(0.6, Number(currentMood?.liarBias ?? 0)));
    if (Math.random() < p){
      let other = baseIdx;
      let guard = 0;
      while (other === baseIdx && guard++ < 10){
        other = randInt(1, HEADSHOT_COUNT);
      }
      idPhotoSrc = headshotPath(other);
    }
    return { facePhotoSrc, idPhotoSrc };
  }

  function syncVisitorAvatars(){
    if (!state?.visitor) return;

    // Avatar face photo = what you see in portrait/avatar
    const face = state.visitor.facePhotoSrc || state.visitor.photoSrc || "";
    const doc  = state.visitor.idPhotoSrc || face;

    if (portraitPhoto) portraitPhoto.src = face || TRANSPARENT_PX;
    visitorAvatar.src = face || TRANSPARENT_PX;

    // Legacy ID slot photo (document photo)
    if (idPhoto) idPhoto.src = doc || TRANSPARENT_PX;

    // Training ID card photo (document photo)
    if (idPhoto2) idPhoto2.src = doc || TRANSPARENT_PX;
  }

  // -------- Training ID card fill --------
  function fillTrainerIdCard(){
    if (!trainerIdCard || !state?.visitor) return;
    ...
    // photo: document photo (can differ from face)
    if (idPhoto2) idPhoto2.src = (v.idPhotoSrc || v.facePhotoSrc || v.photoSrc || TRANSPARENT_PX);

    if (idName2) idName2.textContent = v.name || "";
    if (idDob2) idDob2.textContent = v.dob || "";
    if (idNat2) idNat2.textContent = v.nat || "";
    if (idNo2) idNo2.textContent = v.idNo || "";

    // Optional fields for training
    if (idReason2) idReason2.textContent = "Appointment";
    if (idExp2) idExp2.textContent = "Today 23:59";

    // Barcode/QR payload string (internal)
    if (idBarcode2){
      const payload = `VEVA|${v.idNo || ""}|${v.last || ""}|${v.nat || ""}`.toUpperCase();
      idBarcode2.textContent = payload;
      idBarcode2.setAttribute("data-payload", payload);
    }

    trainerIdCard.hidden = false;
    trainerIdCard.style.display = "";
  }

  function hideTrainerIdCard(){
    if (!trainerIdCard) return;
    trainerIdCard.hidden = true;
    trainerIdCard.style.display = "none";
  }
  ...
  function showId(){
    ...
    // Legacy fields (keep working; you can remove later if you want)
    ...
    // Legacy photo = document photo
    const doc = state.visitor.idPhotoSrc || state.visitor.facePhotoSrc || state.visitor.photoSrc || TRANSPARENT_PX;
    if (idPhoto) idPhoto.src = doc;
    ...
    // Fill & show training card (inside same wrapper)
    fillTrainerIdCard();
  ...
  function resetScenario(){
    ...
    // Build face/doc photo pair (doc can differ sometimes)
    const pair = buildPhotoPair(ID_DATA.headshotIndex, ID_DATA.photoSrc);
    ...
    state = {
      ...
      // IMPORTANT:
      // - facePhotoSrc is used for the avatar/portrait
      // - idPhotoSrc is used for the ID card (can be different sometimes)
      visitor: { ...ID_DATA, facePhotoSrc: pair.facePhotoSrc, idPhotoSrc: pair.idPhotoSrc },
  ...
  state = {
    stage: "idle",
    typing: { visitor:false, student:false },
    visitor: { ...ID_DATA, facePhotoSrc: ID_DATA.photoSrc, idPhotoSrc: ID_DATA.photoSrc },
    facts: {},
    misses: 0
  };
  syncVisitorAvatars();
